...
<!--  Includes mode "metadata" for front matter, along with 
  ...
  <xsl:template match="author-notes/corresp" mode="metadata">
    <xsl:call-template name="metadata-labeled-entry">
      <xsl:with-param name="label">
        <xsl:call-template name="named-anchor"/>
        <xsl:text>Correspondence</xsl:text><!--JS: Text changed to reflect NPG practice-->
      </xsl:with-param>
    </xsl:call-template>
  </xsl:template>
...
<!-- ============================================================= -->
  ...
  <xsl:template priority="2" mode="list"
    match="list[@list-type='none' or list-item/label]"><!--JS - inserted NPG @list-type value-->
  ...
  <xsl:template match="list" mode="list">
    <xsl:variable name="style">
      <xsl:choose><!--JS - inserted NPG @list-type values-->
        <xsl:when test="@list-type='lcletter'">lower-alpha</xsl:when>
        <xsl:when test="@list-type='ucletter'">upper-alpha</xsl:when>
        <xsl:when test="@list-type='lcroman'">lower-roman</xsl:when>
        <xsl:when test="@list-type='ucroman'">upper-roman</xsl:when>
...
<xsl:template match="xref[not(normalize-space(string(.)))]">
  <a href="#{@rid}">
    <xsl:variable name="item" select="key('element-by-id',@rid)"/>
    <xsl:choose>
    <xsl:when test="local-name($item) = ('aff','corresp')">
    <xsl:call-template name="make-label-text">
      <xsl:with-param name="auto" select="true()"/>
      <xsl:with-param name="auto-text">
      <span><xsl:value-of select="$item/@id"/></span>
      </xsl:with-param>
      <xsl:with-param name="warning" select="true()"/>
    </xsl:call-template>
    </xsl:when>
    <xsl:otherwise>
    <xsl:apply-templates select="$item" mode="label-text">
      <xsl:with-param name="warning" select="true()"/>
    </xsl:apply-templates>
    </xsl:otherwise>
    </xsl:choose>
  ...
  <xsl:variable name="auto-label-fig" select="not(//fig[label])"/><!--JS - added auto labelling for figures-->
  ...
  <xsl:variable name="auto-label-supplementary"
    select="false()"/>
  <xsl:variable name="auto-label-table-wrap" select="not(//table-wrap[label])"/><!--JS - added auto labelling for tables-->
  ...
  <xsl:template match="table-wrap" mode="label-text">
    ...
    <xsl:call-template name="make-label-text">
      ...
      <xsl:with-param name="auto-text">
        ...
        <xsl:number level="any"/><!--JS - deleted roman numeral numbering of tables: @format="I"-->
      </xsl:with-param>
    </xsl:call-template>
  </xsl:template>
...
<!-- ============================================================= -->
  ...
  <xsl:template name="metadata-labeled-entry">
	...
	<xsl:if test="local-name(.)='corresp'">
          <span class="generated">[</span><xsl:value-of select="@id"/><span class="generated">] </span>
	</xsl:if>
        <xsl:copy-of select="$contents"/>
      </xsl:with-param>
    </xsl:call-template>
